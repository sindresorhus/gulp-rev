...
it('should reuse an existing manifest file if specified', function(cb) {
	var stream = rev.manifest({
    existingManifest: './test/rev-manifest.json'
  });

	stream.on('data', function (newFile) {
		var MANIFEST = {};

		MANIFEST[path.join('baz', 'lochness.css')] = path.join('foo', 'lochness-d41d8cd9.css');
		MANIFEST[path.join('foo', 'unicorn.css')] = path.join('foo', 'unicorn-d41d8cd9.css');
		MANIFEST[path.join('bar', 'pony.css')]    = path.join('bar', 'pony-d41d8cd9.css');
...
it('should reuse an existing manifest object if specified', function(cb) {
	var stream = rev.manifest({
    existingManifest: {
      'baz/lochness.css': 'foo/lochness-d41d8cd9.css'
    }
  });
